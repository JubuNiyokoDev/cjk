...
import { Image as ImageIcon, Video, X, Play, Plus, Trash2, GripVertical, Edit2, Check, Eye, EyeOff } from 'lucide-react';
...
function SortableGalleryItem({ item, onEdit, onDelete, onToggleVisibility }: { item: GalleryItem; onEdit: (item: GalleryItem) => void; onDelete: (item: GalleryItem) => void; onToggleVisibility: (item: GalleryItem) => void }) {
    ...
    >
      <>
          ...
          <button
            ...
            title={item.is_public === false ? 'Rendre public' : 'Masquer du public'}
            className={`absolute top-2 right-[5.5rem] z-20 p-2 rounded-full ${item.is_public === false ? 'bg-slate-600 hover:bg-slate-700' : 'bg-green-600 hover:bg-green-700'}`}
          >
            {item.is_public === false ? <EyeOff className="w-4 h-4 text-white" /> : <Eye className="w-4 h-4 text-white" />}
          </button>
      ...
      <div className={`${heightClasses[item.height]} w-full relative`} style={{ aspectRatio: '4/5' }}>
        ...
        {item.is_public === false && (
          <div className="absolute top-14 right-4 px-3 py-1 bg-slate-900/80 backdrop-blur-sm rounded-full text-xs font-semibold text-white flex items-center gap-1.5">
...
export default function AdminGalleryPage() {
  ...
  const handleToggleVisibility = async (item: GalleryItem) => {
    ...
    if (!itemId) return;
    const wasPublic = item.is_public !== false;
    // Mise à jour optimiste
    setItems((prev) => prev.map((i) => (getItemId(i) === itemId ? { ...i, is_public: !wasPublic } : i)));
    try {
      await safeFetch(`/api/social/gallery/${itemId}/toggle_visibility/`, undefined, { method: 'POST' });
      toast({ title: 'Succès', description: wasPublic ? 'Média masqué du public.' : 'Média rendu public.' });
    } catch (error) {
              ...
              <SortableContext items={items.map((item) => getItemId(item))} strategy={rectSortingStrategy}>
                <motion.div layout className="columns-1 md:columns-2 lg:columns-3 gap-2 space-y-3">
                  {filteredItems.map((item) => (
                    <SortableGalleryItem key={getItemId(item)} item={item} onEdit={openEditModal} onDelete={handleDelete} onToggleVisibility={handleToggleVisibility} />
                  ))}
                </motion.div>
              </SortableContext>
            </DndContext>
